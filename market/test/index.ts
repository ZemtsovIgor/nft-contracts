...
const B8DEXNFTMarket = artifacts.require("./B8DEXNFTMarketV1.sol");
const MainCollectionChecker = artifacts.require("./MainCollectionWhitelistChecker.sol");
...
contract(
  ...
  ([owner, admin, treasury, buyer1, buyer2, buyer3, seller1, seller2, seller3, creator1]) => {
    ...
    // Contracts
    let collectibleMarket: any;
    let mockERC20: any;
    let mockNFT1: any;
    let mockNFT2: any;
    let mockNFT3: any;
    let mockNFT4: any;
    let mainCollection: any;
    let mainCollectionChecker: any;
    let wrappedBNB: any;
    ...
    before(async () => {
      ...
      collectibleMarket = await B8DEXNFTMarket.new(
        admin,
        treasury,
        wrappedBNB.address,
        minimumAskPrice,
      ...
      // Deploy B8DEXMainCollection (modified implementation in Solidity 0.8)
      mainCollection = await B8DEXMainCollection.new({ from: owner });

      // Deploy mainCollectionChecker
      mainCollectionChecker = await MainCollectionChecker.new(mainCollection.address, { from: owner });
    ...
    describe("COLLECTIBLE MARKET #3 - TOKEN RESTRICTIONS/mainCollection", async () => {
      it("Add collection with restrictions", async () => {
        const result = await collectibleMarket.addCollection(
          mainCollection.address,
          constants.ZERO_ADDRESS,
          mainCollectionChecker.address,
          "100", // 1%
          "0",
          { from: admin }
        );
        ...
        expectEvent(result, "CollectionNew", {
          collection: mainCollection.address,
          creator: constants.ZERO_ADDRESS,
          whitelistChecker: mainCollectionChecker.address,
          tradingFee: "100",
          creatorFee: "0",
        });

        assert.equal(await mainCollectionChecker.canList("1"), true);
        assert.equal(await mainCollectionChecker.canList("2"), true);
        assert.equal(await mainCollectionChecker.canList("3"), true);
        assert.equal(await mainCollectionChecker.canList("4"), true);
        assert.equal(await mainCollectionChecker.canList("211"), true);

        const tokenListingStatuses = await collectibleMarket.canTokensBeListed(mainCollection.address, [
          "0",
          "1",
          "2",
          "3",
      ...
      it("Owner mint collectionId 1-5 for seller1 and owner adds restrictions for collectionId 3/4", async () => {
        let i = 0;

        while (i < 5) {
          await mainCollection.mint(seller1, "ipfs://" + i.toString(), i, { from: owner });
          i++;
        }

        const result = await mainCollectionChecker.addRestrictionForCollection([new BN("3"), new BN("4")]);
        expectEvent(result, "NewRestriction");

        assert.equal(await mainCollectionChecker.isCollectionIdRestricted("3"), true);
        assert.equal(await mainCollectionChecker.isCollectionIdRestricted("4"), true);

        // For convenience, tokenId = 0 --> collectionId = 0, tokenId = 1 --> collectionId = 1
        assert.equal(await mainCollectionChecker.canList("3"), false);
        assert.equal(await mainCollectionChecker.canList("4"), false);

        const tokenListingStatuses = await collectibleMarket.canTokensBeListed(mainCollection.address, ["3", "4"]);
        const boolArray = Array.from({ length: 2 }, (i) => (i = false));
        assert.sameOrderedMembers(tokenListingStatuses, boolArray);
      });

      it("Seller 1 can sell tokenIds 0-2 (collectionIds 0-2)", async () => {
        await mainCollection.setApprovalForAll(collectibleMarket.address, true, { from: seller1 });

        let result = await collectibleMarket.createAskOrder(mainCollection.address, "0", parseEther("1"), {
          from: seller1,
        });
        ...
        result = await collectibleMarket.createAskOrder(mainCollection.address, "1", parseEther("1"), {
          from: seller1,
        });
        ...
        result = await collectibleMarket.createAskOrder(mainCollection.address, "2", parseEther("1"), {
          from: seller1,
        });
      ...
      it("Seller 1 cannot sell tokenIds 3-4 (collectionIds 3-4)", async () => {
        await expectRevert(
          collectibleMarket.createAskOrder(mainCollection.address, "3", parseEther("1"), {
            from: seller1,
          }),
        ...
        await expectRevert(
          collectibleMarket.createAskOrder(mainCollection.address, "4", parseEther("1"), {
            from: seller1,
          }),
          "Order: tokenId not eligible"
        );
      ...
      it("Owner removes restrictions for collectionId=4", async () => {
        let result = await mainCollectionChecker.removeRestrictionForCollection([new BN("4")]);
        expectEvent(result, "RemoveRestriction");

        assert.equal(await mainCollectionChecker.isCollectionIdRestricted("3"), true);
        assert.equal(await mainCollectionChecker.isCollectionIdRestricted("4"), false);
        assert.equal(await mainCollectionChecker.canList("3"), false);
        assert.equal(await mainCollectionChecker.canList("4"), true);
        ...
        result = await collectibleMarket.createAskOrder(mainCollection.address, "4", parseEther("1"), {
          from: seller1,
        });
      ...
      it("Revert statements work as expected", async () => {
        await expectRevert(
          mainCollectionChecker.removeRestrictionForCollection([new BN("3"), new BN("4")], { from: owner }),
          "Operations: Not restricted"
        );

        await expectRevert(
          mainCollectionChecker.addRestrictionForCollection([new BN("3"), new BN("4")], { from: owner }),
    ...
    describe("COLLECTIBLE MARKET #4 - ADMIN/OWNER/SPECIAL BEHAVIOR", async () => {
      ...
      it("Cannot deploy if wrong admin address/treasury and maxPrice <= minPrice", async () => {
        ...
        await expectRevert(
          B8DEXNFTMarket.new(admin, constants.ZERO_ADDRESS, wrappedBNB.address, minimumAskPrice, maximumAskPrice, {
            from: owner,
          }),
          "Operations: Treasury address cannot be zero"
        );
        ...
        await expectRevert(
          B8DEXNFTMarket.new(admin, treasury, constants.ZERO_ADDRESS, minimumAskPrice, maximumAskPrice, {
            from: owner,
          }),
        ...
        await expectRevert(
          B8DEXNFTMarket.new(admin, treasury, wrappedBNB.address, minimumAskPrice, minimumAskPrice, {
            from: owner,
          }),
          "Operations: _minimumAskPrice < _maximumAskPrice"
        );
